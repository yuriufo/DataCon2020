...
def check_crypto_by_static(yc, file_info, is_path=True):
    '''基于特征码的加密识别
...
if __name__ == '__main__':
    ...
    yara_rules_path = "../rules/crypto_index.yar"
    yc = YaraCheck(rule_path=yara_rules_path)

    # 方法一：调用识别函数，用文件路径
    #check_suss, results = check_crypto_by_static(yc, "./reverse1_final.exe")
    ...
    for root, dirs, files in os.walk("../1_2000_black/"):
        ...
        for file_name in files:
          ...
          check_suss, results = check_crypto_by_static(yc, bdata, False)
